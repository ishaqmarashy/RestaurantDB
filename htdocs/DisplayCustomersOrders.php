...
$countQ = "select * from customerorder where customerid='$cID'";
$tempc = runQuery($connection,$countQ);
$NF = mysqli_num_rows($tempc);
$countQ = "select * from customerorder where customerid='$cID'and fulfilled=0";
$tempc = runQuery($connection,$countQ);
$UN = mysqli_num_rows($tempc);

$query = "select fulfilled,firstname,lastname,address,c.orderdate,
c.ordernumber,b.productid,b.quantity,e.restaurantname,e.productname
 from customerinfo a, orderinfo b, customerorder 
 c,product e where a.customerid='$cID'
  and e.productid=b.productid and 
  a.customerid=c.customerid and c.ordernumber=b.ordernumber ORDER BY FULFILLED ASC ";
$result = runQuery($connection,$query);
if ($NF == 0) 
{
	print "No customer with ID = $cID";
	print "</body></html>";
	exit();
}
print "<h3>Customer with ID = $cID</h3>";
print "<h3>Total number of orders =$NF </h3>";
print "<h3>Total number of unfulfilled orders =$UN </h3>";
displayTable($result);
?>
