...
$ON = $_POST['ON'];
$VC = $_POST['VC'];
$q1 = "SELECT *  FROM Voucher WHERE CODE='$VC'";
$result = runQuery($connection,$q1);
if ($result)
{
	$p1= mysqli_fetch_array($result);
	echo p1[2];
	$q2 = "SELECT * FROM ORDERINFO a,PRODUCT b WHERE a.productid=b.productid and restaurantname='$p1[2]' AND ordernumber =$ON";
	
	$result2 = runQuery($connection,$q2);
	if($result2){
		$q3 = "SELECT value FROM CUSTOMERORDER WHERE ordernumber =$ON";
		$result3 = runQuery($connection,$q3);
		if($result3){
			$p3 = mysqli_fetch_array($result3);
			$val = $p1[1] * (0.05);
			if($val>$p3[0]){
				$q3 = "UPDATE CUSTOMERORDER SET VALUE=0 WHERE ordernumber =$ON";
				$result3 = runQuery($connection,$q3);
				$q3 = "DELETE FROM Voucher WHERE CODE ='$VC'";
				runQuery($connection,$q3);
				
			}
			else {
				$q3 = "UPDATE CUSTOMERORDER SET VALUE=VALUE-$val WHERE ordernumber =$ON";
				$result3 = runQuery($connection,$q3);
				$q3 = "DELETE FROM Voucher WHERE CODE ='$VC'";
				runQuery($connection,$q3);
			}
