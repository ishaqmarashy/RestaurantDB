...
$connection = dbConnect("$username", "$password") ;
  ...
  $queryR ="SELECT POINTS FROM LOYALTY WHERE CUSTOMERID=$ID AND RESTAURANTNAME='$STO'";
  $R =  mysqli_query($connection,$queryR);
  $ANS = mysqli_fetch_array($R);
  $numrows = mysqli_num_rows($R);
if ($numrows != 0)
{
  echo '<p style="color:red;">'.$rand .'</p><br/>';
  $P = 0;
  $P = $ANS[0];
  
  $query="INSERT INTO Voucher VALUES ('$rand',$P,'$STO')";
  echo $query;
  $queryD ="UPDATE LOYALTY SET POINTS=0 WHERE CUSTOMERID=$ID AND RESTAURANTNAME='$STO'";
  $a = mysqli_query($connection,$queryD);
  print($a);
