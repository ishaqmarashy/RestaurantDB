...
$result = runQuery($connection,$q1);
$p = mysqli_fetch_array($result);
$v = $QT*$p[0];
$q2 = "UPDATE CUSTOMERORDER SET Value=Value+'$v' WHERE ORDERNUMBER = $ON";
$insResult = mysqli_query($connection,$query);
if ($insResult)
{
	print("Details for " . $ON . " ".$ID."" . $QT . " have been inserted<br/>");
	echo mysqli_query($connection,$q2);
	...
	$query2 = "select restaurantname from loyalty where RESTAURANTNAME= '$finfo[8]' and CUSTOMERID= '$finfo[1]'";
	$result2 = mysqli_query($connection,$query2);
	if(mysqli_num_rows($result2)==0){
		$query3 = "insert into loyalty values('$finfo[8]',0,'$finfo[1]')";
		$result3 = mysqli_query($connection,$query3);
		}
	if($gen==M){
		$query = "update loyalty set points=points+'$finfo[9]' where customerid='$finfo[1]' and restaurantname='$finfo[12]'";
		$result = mysqli_query($connection,$query);	$temp=mysqli_fetch_array($result);
	print("The given id is " . $temp[0] . ".<br/>");}
	else{
		$query = "update loyalty set points=points+'$finfo[10]' where customerid='$finfo[1]' and restaurantname='$finfo[12]'";
		$result = mysqli_query($connection,$query);
