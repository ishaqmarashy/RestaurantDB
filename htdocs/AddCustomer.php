...
$connection = dbConnect("$username", "$password") ;
  ...
  $gen = $_POST['gen'];
$query =  "INSERT INTO CUSTOMERINFO VALUES (NULL,'$first', '$last', '$add', 
                       '$email', '$doB', '$regdate','$gen' )";
$insResult = mysqli_query($connection,$query);
if ($insResult)
{
    print("Details for " . $first . " " . $last . " have been inserted<br/>");
	$query =  "SELECT CUSTOMERID FROM CUSTOMERINFO WHERE FIRSTNAME='$first' AND LASTNAME='$last' AND EMAIL='$email'";
	$insResult = mysqli_query($connection,$query);
	$temp=mysqli_fetch_array($insResult);
